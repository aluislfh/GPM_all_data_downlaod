...
def main():
    ...
    lista = sorted(glob.glob('3B-HHR-E.MS.MRG.3IMERG.*.V06B.HDF5'))
    ...
    fplot(map1,'cuba',theLats,theLons,rain,lista)
...
def fplot(map,label,theLats,theLons,rain,lista):
    ...
    # map.readshapefile('/opt/productos/tracks/shp/reproj_prov', 'roads', color='k', linewidth=0.6)
    # map.readshapefile('/opt/productos/tracks/shp/reproj_muni', 'municipios', color='grey', linewidth=0.1)
...
#    plt.title('GPM-Imerg product Valid: 2016/01/16 0000 to 2016/01/17 0000', \
...
#        stretch='normal', family='sans-serif')
    ...
    plt.title('PRODUCTO DE ESTIMACION MULTISATELITE DE LA PRECIPITACION (GPM_3IMERGDE)\n ACUMULADOS EN '+nh+' HORAS (MM/'+nh+'H) ENTRE '+str(sutc)[:-3]+' - '+str(eutc)[:-3]+ ' [UTC]',fontsize=8)

    plt.savefig(label+'.png',bbox_inches='tight',dpi=300)
    plt.clf()
    plt.cla()
    plt.close('all')
